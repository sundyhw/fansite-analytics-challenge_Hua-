# From Hua Wei
Note that in feature 3, the 60 minites time peroid needs more stringent definition. 
There are two ways to define: 1) 0 second <=time <= 3600 second or 2) 0 second <=time  < 3600 second .
I use the 1). Using 1) and 2) will yield different results.

# Additional feature 1 
Feature engineering is important for malicious users/IPs/activities detection.
 By adding more features in modeling, we can increase the model complexity and prediction accuracy. The  Number (of failed login), Number (of overall login), and ratio for each IP could serve as features for determining malicious users. Consider malicious users are more likely to have failed attempts. 
...
The results are output in `feature4.txt`. It is interesting that the this output is empty, whichs mean after three attemps, this IP doesn't show successful login attempt.
